...
double pccDelay = 1 * 1000; //!< 1000ms (1s)
...
std::string path = "./";
std::string sfa = ""; //!< ["isfa", "csfa", "cesfa", "asfa"]
...
std::vector<double> delayPerApp(2, 0.0); //!< { 0: 'IMR', 1: 'PCC' }
...
std::string MakeFileName(std::string name, std::string extension = "csv") 
{
  std::string fileName = path + "/" + std::to_string(nGateways) + "gw_" 
                              + (adrEnabled ?  adrName + "_" : "") 
                              + (sfa != "" ? sfa + "_" : "") 
                              + txMode + "_" + name + "." + extension;
  return fileName;
}
...
void PrintSFAndTP()
{
  // SF7, SF8, SF9, SF10, SF11, SF12, TP2, TP4, TP6, TP8, TP10, TP12, TP14
  std::ostringstream oss;
...
main(int argc, char* argv[])
{
    ...
    if (adrEnabled)
    {
      ...
      if (adrName == "adr")
      {
        ...
        Config::SetDefault("ns3::AdrComponent::MultiplePacketsCombiningMethod", EnumValue(AdrComponent::MAXIMUM));    
      }
      else if (adrName == "ca_adr")
      {
        Config::SetDefault("ns3::AdrComponent::HistoryRange", IntegerValue(20));
        Config::SetDefault("ns3::AdrComponent::MultiplePacketsCombiningMethod", EnumValue(AdrComponent::AVERAGE));
        Config::SetDefault("ns3::AdrComponent::MultipleGwCombiningMethod", EnumValue(AdrComponent::MAXIMUM));
        Config::SetDefault("ns3::CAADR::Interval", DoubleValue(600));
        Config::SetDefault("ns3::CAADR::Run", IntegerValue(nRun));
      }
      else if (adrName == "mb_adr")
      {
        Config::SetDefault("ns3::AdrComponent::HistoryRange", IntegerValue(20));
      }
    }
    ...
    {
      LorawanMacHelper::CeSFA(endDevices, gateways, channel, false);
    } 
    else if (sfa == "rsfa")
    {
    ...
    {
      LorawanMacHelper::ITPA(endDevices, gateways, channel, true);
    }
    else if (sfa == "drsfa")
    {
      ...
      LorawanMacHelper::DRSFA(endDevices, gateways, channel, data, nRun, true);
